    ...
    for root, _, files in sorted(os.walk(data_dir)):
        for filename in sorted(files):
            if filename.endswith(".asc"):
                ...
                # Extract the first 4 letters for sub
                sub = re.search(r"^([A-Za-z]{4})", filename).group(1)

                # Extract the last 2 or 3 digits before the .asc extension for proba
                proba = int(re.search(r"(\d{2,3})\.asc$", filename).group(1))

                print(f"sub: {sub}, proba: {proba}")
                df["proba"] = proba
                df["sub"] = sub
